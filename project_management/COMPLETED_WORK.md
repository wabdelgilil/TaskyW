...
- **صوت بشري أصلي من الذكاء الاصطناعي (Generative Speech عبر Gemini 2.5 Flash TTS)**:
  ...
  - ربط المساعد بنموذج `gemini-2.5-flash-preview-tts` بتضمين استجابة الصوت `responseModalities: ["AUDIO"]` لتوليد صوت بشري ناطق بالحياة والمشاعر والانفعالات الطبيعية (24000Hz PCM mono) متوافق مع لهجات وسياقات اللغة العربية.
  - تحويل تدفق الصوت إلى صيغة WAV نقية فائقة الجودة في الذاكرة بدون أي اعتمادية على نظام الملفات.
- **مشغل صوتي عابر للمنصات (Cross-Platform Generative Audio Player)**:
  - اعتماد حزمة `audioplayers` لتشغيل الصوت البشري فورياً عبر الذاكرة عبر متصفحات الويب (Web)، الويندوز (Windows)، والموبايل (Android & iOS).
  - مزامنة فورية لحالات المشغل الصوتي (`speaking`, `idle`, `stopSpeaking`).
- **تخصيص شخصيات ونبرات الصوت (AI Voice Personas)**:
  - إضافة خيار اختيار شخصية الصوت في الإعدادات: `Puck` (حيوي وطبيعي)، `Aoede` (هادئ وواضح)، `Kore` (ودود ومتزن)، `Fenrir` (عميق ورصين).
- **الفحص والتحقق**:
  - `flutter analyze`: **0 issues found!** بنجاح كامل.
  - اختبارات الوحدة والواجهات: **20/20 اختباراً ناجحاً بنسبة 100%**.
  - ترقية الإصدار إلى **v3.3.3+1** في `pubspec.yaml` و `app_version.dart`.

### [2026-09-17] - الملخص الصوتي التنفيذي الذكي للمهام بواسطة الذكاء الاصطناعي (v3.3.2+1)
- **تحويل استعراض المهام إلى ملخص تنفيذي ذكي (AI Executive Voice Briefing)**:
  - استبدال السرد الآلي الرتيب للمهام (الذي كان يقرأ كروت المهام بشكل ميكانيكي مجرد) بملخص تحليلي تنفيذي طبيعي وجذاب تصيغه نماذج Gemini بالعامية المصرية الراقية أو العربية الحديثة كأنه سكرتير تنفيذي ذكي.
  - تمرير قائمة المهام النشطة (حتى 40 مهمة) إلى الـ System Context في `GeminiVoiceService` مع تفاصيل كل مهمة (المعرف، العنوان، موعد الاستحقاق، الأولوية، اسم المشروع والمجال).
  - يقوم الذكاء الاصطناعي بتحليل الجدول، وفرز الأولويات، والبدء بالمهام العاجلة ومواعيد التسليم القريبة مع تنبيه المستخدم، وتقديم توجيهات ونصائح إنتاجية وسؤال تفاعلي لطيف.
  - نطق الملخص الصوتي بالكامل عبر محرك الصوت `FlutterTts` وعرض النص التحليلي مباشرة في واجهة المساعد `AiVoiceAssistantSheet`.
- **الفحص والتحقق**:
  - `flutter analyze`: **0 issues found!** عبر كامل المشروع.
  - اختبارات الوحدة والواجهات: **20/20 اختباراً ناجحاً** (AI Assistant, Settings, Widgets).
  - ترقية الإصدار إلى **v3.3.2+1** في `pubspec.yaml` و `app_version.dart`.

### [2026-09-17] - إصلاح استجابة أزرار وميكروفون المساعد الذكي على الويب وكافة المنصات (v3.3.1+2)
- **إصلاح عدم استجابة الأزرار والاقتراحات السريعة (Fix Button Responsiveness)**:
  - إزالة التبعية غير الموجودة على `context.read<TasksController>` و `Provider` في شاشة المساعد، والتي كانت تسبب خطأ runtime خفياً في المتصفح يمنع الأزرار من الاستجابة عند النقر عليها.
  - تمرير المشاريع والمجالات والمهام ودالة الحفظ مباشرة من الشاشة الهيكلية `MainLayoutScreen` إلى شريط الرأس `MainTopHeader` ومنه إلى `AiVoiceAssistantSheet`.
- **إصلاح التسجيل الصوتي للميكروفون على الويب (Cross-Platform In-Memory Audio Recording)**:
  - استبدال التخزين في ملفات محلية عبر `dart:io` و `path_provider` (غير المدعومة على متصفح الويب) ببث صوتي فوري في الذاكرة `startStream` بدقة PCM 16-bit 16000Hz.
  - بناء دالة تحويل فوري لملفات WAV في الذاكرة بصيغة `audio/wav` متوافقة مباشرة مع نموذج Gemini API بدون أي اعتمادية على نظام الملفات.
- **الفحص والتحقق**:
  - `flutter analyze`: **0 issues found!**
  - كافة اختبارات المساعد وشاشات التطبيق والإعدادات: **20/20 اختباراً ناجحاً**.

### [2026-09-17] - ترقية محرك الذكاء الاصطناعي لـ Gemini 2.5/3.6 ودعم المفاتيح الحديثة AQ (v3.3.1+1)
- **ترقية النماذج المدعومة وتجاوز إيقاف الموديل القديم (Upgrade to Gemini 2.5 Flash & 3.6 Flash)**:
  - ترقية النموذج الافتراضي في `AppSettingsModel` و `SettingsService` و `GeminiVoiceService` من `gemini-2.0-flash` (الذي أوقفته Google) إلى `gemini-2.5-flash` و `gemini-3.6-flash`.
  - معالجة ذكية للترحيل التلقائي (Migration) لأي إعدادات قديمة مسجلة كـ `gemini-2.0-flash` أو `gemini-1.5-flash` لترقيتها تلقائياً عند التشغيل.
- **دعم وتوافق كامل مع مفاتيح Google الجديدة بتنسيق `AQ.`**:
  - دعم مفاتيح الاستيثاق الأحدث من Google AI Studio التي تبدأ بـ `AQ.Ab...`.
  - تحديث دالة فحص المفتاح `testGeminiApiKeyDetailed` في `SettingsController` لتقوم بفحص آلي متسلسل للنماذج المتوافقة وربط المفتاح بالنموذج الشغال فورياً مع إعطاء رسائل خطأ دقيقة وواضحة بالعربية.
  - تحديث نص التلميح في شاشة الإعدادات ليشمل `(AIzaSy... أو AQ...)`.
- **ترقية الإصدار إلى v3.3.1+1**:
  - تحديث `pubspec.yaml` و `app_version.dart`.
  - كافة الفحوصات `flutter analyze` خالية تماماً من الأخطاء (0 issues).

### [2026-09-17] - المساعد الصوتي الذكي ونظام المفتاح الشخصي BYOK (v3.3.0+1)
- **المساعد الصوتي والذكاء الاصطناعي الشخصي (TaskyW AI Voice Assistant)**:
  - إضافة موديول ذكاء اصطناعي متكامل في `lib/features/ai_assistant/` يدعم أوامر الصوت الطبيعية باللهجات العربية والفصحى والإنجليزية لمعالجة الأوامر في ثوانٍ معدودة.
  - **إنشاء المهام صوتياً (Voice Task Creation)**: استخراج تفاصيل المهمة (العنوان، الموعد المحسوب بدقة، الأولوية، وربطها التلقائي بالمشروع أو المجال المطابق)، مع إنشاء المهمة محلياً ونطق رسالة تأكيد صوتية للمستخدم.
  - **قراءة واستعراض المهام صوتياً (Voice Task Reading & Queries)**: تنفيذ أوامر القراءة الصوتية مثل *"اقرأ مهام اليوم"*، *"اقرأ مهام مشروع كذا"*، *"اقرأ كل المهام النشطة"* عبر ربطه بمحرك النطق الصوتي المدمج `AudioBriefingController` مع تظليل تفاعلي للكروت على الشاشة.
  - **تسجيل صوتي عابر للمنصات (Cross-Platform Audio Recording)**: اعتماد حزمة `record` لتسجيل مقاطع الصوت عبر الميكروفون على أنظمة Windows و Android و Web، وتمرير الصوت لنموذج Gemini 2.0 Flash مباشرة.
- **نظام المفتاح الشخصي (Bring Your Own Key - BYOK)**:
  - تصميم قسم كامل في شاشة الإعدادات `SettingsScreen` بعنوان **الذكاء الاصطناعي والمساعد الصوتي**.
  - حقل آمن لإدخال وحفظ مفتاح Google Gemini API في `SharedPreferences` محلياً مع زر إظهار/إخفاء وزر لصق فوري وزر مسح.
  - زر **اختبار الاتصال (Test Connection)** لفحص صلاحية المفتاح فورياً مع رد بصري (علامة خضراء عند النجاح أو رسالة توضيحية عند الفشل).
  - رابط مباشر وسريع لفتح Google AI Studio لاستخراج المفتاح مجاناً في ثوانٍ.
  - توجيه لطيف للمستخدم عند فتح المساعد الصوتي بدون مفتاح مع زر مباشر للإعدادات.
- **الواجهات والتجاوب (UI/UX & Responsiveness)**:
  - نافذة المساعد السفلية `AiVoiceAssistantSheet` مع موجات صوتية نابضة، وزر ميكروفون تفاعلي، وأزرار اقتراحات سريعة، وخيار إدخال نصي بديل.
  - إضافة زر المساعد الصوتي السحري (`Icons.auto_awesome_rounded`) في شريط الرأس العلوي `MainTopHeader`.
  - تحسين الاستجابة المكانية لشريط الرأس ومنع أي فيض (Overflow) عبر إتاحة وضعية العرض المضغوط للأزرار على الشاشات المتوسطة وعرض شريط التمرير الأفقي للأزرار على الموبايل.
- **الفحص والاختبارات**:
  - `flutter analyze`: **0 issues found!** بنجاح كامل.
  - إضافة 10 اختبارات شاملة في `test/ai_assistant_test.dart` لتغطية استخراج الـ JSON، وإدارة المفتاح، وحالات المتحكم.
  - نجاح كافة اختبارات الودجت في `test/widget_test.dart` (Desktop, Mobile, OLED) واختبارات الإعدادات `test/settings_screen_test.dart`.
  - ترقية إصدار التطبيق إلى **v3.3.0+1** في `pubspec.yaml` و `app_version.dart`.

### [2026-09-16] - تقسيم جدول المهام بناءً على المشاريع + إصلاح فيض رأس الموبايل (v3.2.2+1)
- **تقسيم جدول المهام لعرض جداول متعددة بحسب المشاريع (Multi-Table Grouping by Project in TasksTableView)**:
  - إضافة خيار التجميع بحسب المشروع (`_canGroupByProject`) في وضعية عرض الجدول (`TasksTableView`) مع شريط تبديل علوي متناسق تماماً مع قائمة المهام.
  - عند تفعيل التجميع، يتم تقسيم المهام إلى جداول بيانات منفصلة (`DataTable`) لكل مشروع له مهام نشطة أو مكتملة، بالإضافة لجدول مخصص للمهام المستقلة غير المنسوبة لمشروع (`بدون مشروع`).
  - رأس قسم مخصص لكل مشروع يعرض أيقونة المشروع التعبيرية (`iconEmoji`)، واسمه، وحالة الإشعارات، وعداد المهام المنجزة وإجمالي المهام مع النسبة المئوية (`done/total (%)`) وشريط تقدم ملون فائق الأناقة.
  - إمكانية طي وفرد أي جدول مشروع أو جدول المهام غير المنسوبة بنقرة واحدة على رأس القسم لسهولة التركيز على مشاريع محددة.
  - إظهار النطاق/المجال (`Area`) بذكاء في خلايا الجدول المخصص للمشروع لمنع التكرار اللفظي غير المفيد لاسم المشروع، مع الحفاظ على تناسق الأعمدة والفرز التفاعلي.
  - تحسين لون تحويم صفوف الجدول ليتوافق مع معايير التباين العالي.
  - إضافة اختبارات ودجت شاملة للتجميع والطي والفرد في `test/tasks_table_view_test.dart` بنجاح 5/5.
- **إصلاح فيض عناصر الرأس في شاشات الموبايل (Fix Mobile Top Header RenderFlex Overflow)**:
  - معالجة خطأ `RenderFlex overflowed by 147 pixels on the right` في `MainTopHeader` عند مقاسات الشاشات الضيقة (`< 600px`).
  - تغليف أزرار الإجراءات السريعة (البحث، وضع العرض، تصدير CSV، القراءة الصوتية، حالة المزامنة، والبروفايل) داخل `Flexible` مع `SingleChildScrollView(scrollDirection: Axis.horizontal)` وضبط الكثافة البصرية وحجم الأيقونات بدقة لمنع أي فيض للأبد مهما ضاقت مساحة شاشة الهاتف.
- **الفحص والتحقق والترقية**:
  - `flutter analyze`: **No issues found! (0 errors, 0 warnings)** عبر كامل المشروع.
  - `flutter test test/tasks_table_view_test.dart`: **5/5 passing**.
  - ترقية الإصدار المعتمد إلى `v3.2.2+1` في `pubspec.yaml` و `app_version.dart`.

### [2026-09-16] - مزامنة إعدادات المستخدم محلياً وسحابياً + تمييز فائق التباين لألوان التحويم للكروت (v3.2.1+1)
- **مزامنة الإعدادات محلياً وسحابياً (Local DB & Supabase Cloud Sync)**:
  - إضافة ملف ترحيل Supabase `supabase/migrations/20260916000100_add_task_card_density.sql` لإضافة عمود `task_card_density` لجدول `public.user_settings`.
  - تحديث `SyncService` لرفع وتنزيل إعداد `task_card_density` في عمليتي `_pushUserSettings` و `_pullUserSettings`.
  - إضافة الدالة الفورية `syncSettingsOnly()` في `SyncService` لمزامنة سطر الإعدادات فقط في الخلفية دون الحاجة لمزامنة الجداول الثقيلة.
  - تحديث `SettingsService._mirrorToDb` لتعيين `sync_status = 'pending_update'` عند كل تعديل محلي للإعدادات لتتبع الحاجة للرفع.
  - ربط `SettingsController` في كافة دوال التعديل (`setNotificationsEnabled`, `setDefaultReminderMinutes`, `setDefaultCurrency`, `setDefaultViewMode`, `setThemeMode`, `updateLanguage`, `setLayoutDirection`, `setTaskCardDensity`) بمزامنة الخلفية الفورية عبر `unawaited(SyncService.instance.syncSettingsOnly())`.
  - تحديث `_pullUserSettings` لاستدعاء `SettingsController.instance.load()` فور سحب الإعدادات وتحديث الواجهة مباشرة على كافة الأجهزة عند تسجيل الدخول أو المزامنة التلقائية.
  - إضافة اختبارات وحدة شاملة في `test/settings_sync_test.dart` بنجاح 5/5.
- **تمييز فائق التباين للون التحويم (High-Contrast Hover Effect) لكروت المهام**:
  - تحويل `TaskCard` إلى `StatefulWidget` مع رصد دقيق لتحويم الفأرة عبر `MouseRegion` وحركات انتقالية ناعمة `AnimatedContainer(duration: 140ms)`.
  - **الوضع النهاري (Light Mode)**: عند تمرير المؤشر فوق الكارت الأبيض، يتحول لون الكارت بالكامل إلى داكن كحلي جذاب (`#0F172A`)، وتتحول النصوص والوصف والعناوين للأبيض الناصع (`#FFFFFF`) والرمادي الفاتح (`#CBD5E1`)، ويتحول الإطار إلى سماوي نيون متوهج (`AppColors.brandLight`) مع شارات مضيئة عالية الوضوح.
  - **الوضع الليلي (Dark Slate) ووضع السواد العميق (OLED)**: عند التحويم يرتفع الكارت بلون أردوازي منير (`#334561` أو `#232A36`) مع إطار متوهج ولون نص ناصع البياض.
  - تطبيق التباين العالي على كلا وضعي الكروت (المريح Detailed والـ Compact المضغوط).
  - كتابة اختبارات ودجت شاملة للتحويم والتباين في `test/task_card_hover_test.dart` بنجاح 3/3.
- **الفحص والتحقق**:
  - `flutter analyze`: **No issues found! (0 errors, 0 warnings)**.
  - `flutter test test/task_card_hover_test.dart`: **3/3 passing**.
  - ترقية الإصدار إلى `v3.2.1+1` في `pubspec.yaml` و `app_version.dart`.

### [2026-09-16] - كروت المهام المضغوطة (Compact Task Cards) وتحديث الشريط السفلي (v3.2.0+1)
- **دعم نمط كروت المهام المضغوطة (Compact Task Cards View)**:
  - إضافة دعم خاصية `isCompact` في `TaskCard` لعرض المهمة في سطر واحد مدمج وأنيق بارتفاع ~`44px` يتيح رؤية أكثر من 10 مهام في شاشة الهاتف في نفس الوقت.
  - إظهار شارات مصغرة (Micro-badges) للمشروع والمهام الفرعية وتاريخ الاستحقاق ومؤشر الأولوية اللوني الرفيع مع إخفاء الوصف والتاجات لتوفير الارتفاع والتركيز على الإنتاجية السريعة.
  - إضافة زر تبديل سريع لكثافة العرض (مريح / مضغوط) في رأس قائمة المهام `TaskListView` بجانب خيار التجميع أو كشريط علوي سريع.
  - إضافة خيار "كثافة كروت المهام" في شاشة الإعدادات `SettingsScreen` تحت قسم المظهر والعرض.
  - دعم حفظ واسترجاع تفضيل الكثافة في `AppSettingsModel` و `SettingsService` (عبر SharedPreferences و SQLite).
- **تحديث الشريط السفلي (Bottom Navigation Bar) للهواتف المحمولة**:
  - تقليص وتنظيم الشريط السفلي إلى 4 عناصر وظيفية مباشرة:
    1. **Tasks (المهام)**: لفتح عارض المهام وعرض جميع المهام `all` وإلغاء أي حصر مكاني.
    2. **Notes (الملاحظات)**: لفتح وحدة الملاحظات وقواعد المعرفة (Notes & Vaults).
    3. **Finance (المالية)**: لفتح وحدة المعاملات المالية والتسويات (Finance Module).
    4. **Settings (الإعدادات)**: لفتح شاشة الإعدادات العامة للتطبيق.
  - مزامنة مؤشر الشريط السفلي تلقائياً عند الانتقال من القائمة الجانبية (Drawer).
- **الفحص والتحقق**:
  - `flutter analyze`: **No issues found! (0 errors, 0 warnings)**.
  - `flutter test`: **311/311 tests passing**.
  - ترقية الإصدار إلى `v3.2.0+1` في `pubspec.yaml` و `app_version.dart`.
